...
struct PDR {
    ...
    inline std::pair<uint32_t,uint32_t> sym_bounds(const std::span<SYMR>& symrs, const std::span<AUX>& auxs) {
...
struct FDR {
    ...
    inline uint32_t get_fMerge() {
        return bits >> 26 & 1;
    }

    inline uint32_t get_fReadin() {
        return bits >> 25 & 1;
    }

    inline uint32_t get_fBigEndian() {
        return bits >> 24 & 1;
    }

    inline uint32_t get_glevel() {
        return bits >> 22 & 0b11;
    }
    ...
    inline std::span<AUX> get_auxs(std::vector<AUX>& all_auxs) {
        return std::span(all_auxs).subspan(iauxBase, caux);
    }

    inline std::span<PDR> get_pdrs(std::vector<PDR>& all_pdrs) {
        return std::span(all_pdrs).subspan(ipdFirst, cpd);
    }

    inline std::span<SYMR> get_symrs(std::vector<SYMR>& all_symrs) {
        return std::span(all_symrs).subspan(isymBase, csym);
    }

    inline const char* get_string(const char* data, size_t index) {
        return data + issBase + index;
    }

    inline const char* get_name(const char* data) {
